...
# Copy TradingAgents-CN to writable location (volume is read-only)
if [ ! -d /tmp/TradingAgents ]; then
    cp -r /app/TradingAgents /tmp/TradingAgents
fi

# Install TradingAgents-CN deps on first run (or if pyproject.toml changed)
...
if [ ! -f "$HASH_FILE" ] || [ "$(cat $HASH_FILE)" != "$CURRENT_HASH" ]; then
    ...
    echo ">>> Installing TradingAgents-CN dependencies..."
